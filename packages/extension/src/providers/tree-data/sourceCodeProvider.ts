...
const languageSource: Record<string, string> = {
  cpp: 'run.cpp',
  java: 'Run.java',
  py: 'run.py',
};

const playerCodeDirectory: Record<string, string> = {
  cpp: 'cpp',
  java: 'java',
  py: 'python',
};
              ...
              fs.mkdir(folderPath, err => {
                ...
                const files = fs.readdirSync(
                  path.join(
                    ...
                    'player_code',
                    playerCodeDirectory[folder.language],
                  ),
                );
                ...
                vscode.window.showInformationMessage(
                  `Created file ${fileName}.${folder.language}`,
                );
                vscode.workspace
                  .openTextDocument(
                    vscode.Uri.file(
                      path.join(folderPath, languageSource[folder.language]),
                    ),
                  )
                  .then(doc => {
                    vscode.window.showTextDocument(doc);
