...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (!fs.existsSync(path.join(context.globalStorageUri.fsPath, 'data'))) {
    fs.mkdirSync(path.join(context.globalStorageUri.fsPath, 'data'));
  }
  if (
    !fs.existsSync(
      path.join(context.globalStorageUri.fsPath, 'data', '.vscode'),
    )
  ) {
    fs.mkdirSync(path.join(context.globalStorageUri.fsPath, 'data', '.vscode'));
    fs.writeSync(
      fs.openSync(
        path.join(
